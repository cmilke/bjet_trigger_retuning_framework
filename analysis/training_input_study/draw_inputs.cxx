...
void draw_histograms( vector< vector< vector<TH1*>* >*  > hist_container ) {
    ...
    for (int jettype_index = 0; jettype_index < 3; jettype_index++) {
        for (int plot_index = 0; plot_index < _num_plot_types; plot_index++) {
            ...
            //multihist->SetMinimum(10000);
            //multihist->SetMaximum(200000);
            ...
            for ( vector<float> plot_range : _plot_range_list_array[plot_index] ) {
                ...
                string filename = "plot_"+_value_name[plot_index]+"-"+to_string(range_suffix++)+"_"+to_string(jettype_index)+".pdf";
...
void draw_inputs() {
    ...
    hist_container.push_back( get_hists("hlt", "HLT", kBlue, 7, solid_line) );
    hist_container.push_back( get_hists("ftk", "FTK", kRed, 5, dashed_line) );
