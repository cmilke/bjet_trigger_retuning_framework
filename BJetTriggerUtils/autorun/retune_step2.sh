...
if [[ -n $hybridtuning ]]; then
    ...
    srun --mem=2G --time=10 -p $partition bash ${workingdir}/trainingMV2/inputs/${calibtag_step3}/merge.sh
else
    srun --mem=2G --time=10 -p $partition python mergeFiles.py -dir slurm_arrays/slurm_${date}_${eventtype}_${size2}_step2 -what flavntuple
